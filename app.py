from flask import Flask, Response,render_template,request,url_for,make_response,flash,redirect
...
@app.route("/")
def index():
    # yaha pr we just get the start and end date from query parameters and pass them to the template for now, we will use them later for filtering expenses
    ...
    # we will use these functions to parse the date strings, if the parsing fails we will get None, which means no filtering on that end
    ...
    if start_date and end_date and start_date>end_date:
        ...
        # start_str=end_str="" # we also reset the query parameters to avoid confusion in the UI
...
def add():
    ...
    try:
      ...
      flash("Amount must be a pos number","error")
      return redirect(url_for("index"))
...
def export_csv():
    ...
    for e in expenses:
        lines.append(f"{e.date.strftime('%d-%m-%Y')},{e.description},{e.category},{e.amount:.2f}")


    csv_data="\n".join(lines)
    fname_start=start_str or "start"
    fname_end=end_str or "end"
